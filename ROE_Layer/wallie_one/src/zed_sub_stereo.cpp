...
public:
	...
	subRightRectified(it, "/pcl_reader/right_image_PC", 1),
	subLeftRectified(it, "/pcl_reader/left_image_PC", 1),
	...
	void reconfig( Config& newconfig, uint32_t level )
	{
		...
		if(args[0] != newconfig.A || args[1] != newconfig.B || args[2] != newconfig.C ||args[3] != newconfig.D)
		{	
	...
	void imageStereoRectifiedCallback(const sensor_msgs::Image::ConstPtr&  msgL, const sensor_msgs::Image::ConstPtr&  msgR)
	{
		try
	    {
			...
			resize(dstR, sizedR, Size(int(w/2) * 2, h * 2), INTER_LINEAR);
			right->image = sizedR;
			R = 1;		
	    }
		...
		try
	    {
			...
			resize(dstL,sizedL,Size(int(w/2)*2,h*2),INTER_LINEAR);
			left->image = sizedL;
			L = 1;
	    }
	    catch (cv_bridge::Exception& e)
